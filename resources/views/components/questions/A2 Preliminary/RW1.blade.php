<section id="dropdown-preliminary-writing-1" class="question dropdown dropdown-click closed col-12 col-lg-10 col-xl-8 mb-4">
    <a href="#" class="dropdown-header">
        <span>1.a. Look at the pictures below and compare them thinking about your holiday. Complete these sentences giving your opinion.</span>
        <button class="dropdown-button text-right">
            <i class="dropdown-icon fas fa-chevron-down"></i>
        </button>
    </a>
    <main class="dropdown-menu-content one-question-container px-xl-4">
        <div class="row justify-content-center py-4">
            <div class="card col-12 col-md-6 col-lg-2 mb-4 mb-lg-0">
                <img src="../../img/recursos/A2-PRELIMINARY-W-1.png" class="img-fluid utiles" alt="image-one-question-one">
            </div>
            <div class="card col-12 col-md-6 col-lg-2 mb-4 mb-lg-0">
                <img src="../../img/recursos/A2-PRELIMINARY-W-1.1.png" class="img-fluid utiles" alt="image-one-question-one">
            </div>
            <div class="card col-12 col-md-6 col-lg-2 mb-4 mb-lg-0">
                <img src="../../img/recursos/A2-PRELIMINARY-W-1.2.png" class="img-fluid utiles" alt="image-one-question-one">
            </div>
            <div class="card col-12 col-md-6 col-lg-2 mb-4 mb-lg-0">
                <img src="../../img/recursos/A2-PRELIMINARY-W-1.3.png" class="img-fluid utiles" alt="image-one-question-one">
            </div>
        </div>

        <div class="row d-flex justify-content-between py-4 px-2 px-md-3">
            <p class="complete col-12 mb-4"><b>a. Safe</b> When I go on holiday, I usually travel by plane because it's safer than other transports.</p>
            <p class="complete col-12 mb-4 d-flex justify-content-between"><b>b. Exciting</b><input name="A2_Preliminary:RW1A[1]" class="input input-super-huge text-left" type="text"></p>
            <p class="complete col-12 mb-4 d-flex justify-content-between"><b>c. Hot</b><input name="A2_Preliminary:RW1A[2]" class="input input-super-huge text-left" type="text"></p>
            <p class="complete col-12 mb-4 d-flex justify-content-between"><b>d. Bad</b><input name="A2_Preliminary:RW1A[3]" class="input input-super-huge text-left" type="text"></p>
        </div>
        
        <div class="dropdown-header px-0">
            <span>1.b. What did you do on your last holiday?</span>
        </div>
        <div class="py-4">
            <div class="complete col-12 col-lg-10 col-xl-8 mx-auto position-relative">
                <textarea name="A2_Preliminary:RW1B[1]" class="px-2 text"></textarea>
            </div>
        </div>
    </main>
</section>