<section id="dropdown-access-writing-2" class="question dropdown dropdown-click closed col-12 col-lg-10 col-xl-8 mb-4">
    <a href="#" class="dropdown-header">
        <span>2.a. Fill the gaps in the words, then match the words to the activities shown in the table.<mark>One is done for you</mark>.</span>
        <button class="dropdown-button text-right">
            <i class="dropdown-icon fas fa-chevron-down"></i>
        </button>
    </a>
    <main class="dropdown-menu-content two-question-container px-xl-4">
        <div class="row flex-wrap">
            <table class="table borderless mb-4 mx-auto">
                <tbody>
                    <tr>
                        <th scope="col"><input value="R" disabled maxlength="1" class="input-small input position-relative m-0" type="text">u<input value="n" disabled maxlength="1" class="input-small input position-relative" type="text"></th>
                        <th scope="col" class="input-letters"><select disabled>
                            <option disabled selected>A</option>
                        </select></th>
                    </tr>
                    <tr>
                        <th scope="col" class="input-letters"><input name="A1_Access:RW2A[1]" maxlength="1" class="input-small input position-relative" type="text">rite</th>
                        <th scope="col" class="input-letters"><select name="A1_Access:RW2A[2]">
                            <option disabled selected>a</option>
                            <option>b</option>
                            <option>c</option>
                            <option>d</option>
                            <option>e</option>
                            <option>f</option>
                        </select></th>
                    </tr>
                    <tr>
                        <th scope="col">Dan<input name="A1_Access:RW2A[3]" maxlength="1" class="input-small input position-relative" type="text">e</th>
                        <th scope="col" class="input-letters"><select name="A1_Access:RW2A[4]">
                            <option disabled selected>a</option>
                            <option>b</option>
                            <option>c</option>
                            <option>d</option>
                            <option>e</option>
                            <option>f</option>
                        </select></th>
                    </tr>
                    <tr>
                        <th scope="col">S<input name="A1_Access:RW2A[5]" maxlength="1" class="input-small input position-relative" type="text">im</th>
                        <th scope="col" class="input-letters"><select name="A1_Access:RW2A[6]">
                            <option disabled selected>a</option>
                            <option>b</option>
                            <option>c</option>
                            <option>d</option>
                            <option>e</option>
                            <option>f</option>
                        </select></th>
                    </tr>
                    <tr>
                        <th scope="col">Wat<input name="A1_Access:RW2A[7]" maxlength="1" class="input-small input position-relative" type="text">h</th>
                        <th scope="col" class="input-letters"><select name="A1_Access:RW2A[8]">
                            <option disabled selected>a</option>
                            <option>b</option>
                            <option>c</option>
                            <option>d</option>
                            <option>e</option>
                            <option>f</option>
                        </select></th>
                    </tr>
                    <tr>
                        <th scope="col">Re<input name="A1_Access:RW2A[9]" maxlength="1" class="input-small input position-relative" type="text">d</th>
                        <th scope="col" class="input-letters"><select name="A1_Access:RW2A[10]">
                            <option disabled selected>a</option>
                            <option>b</option>
                            <option>c</option>
                            <option>d</option>
                            <option>e</option>
                            <option>f</option>
                        </select></th>
                    </tr>
                </tbody>
            </table>
    
            <div class="col-lg-8">
                <div class="row row-100 flex-wrap align-content-center">
                    <div class="col-12">
                        <div class="row justify-content-center">
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-2.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto crossed" id="A">
                                    a
                                </div>
                            </div>
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-2.1.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto" id="B">
                                    b
                                </div>
                            </div>
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-2.2.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto" id="C">
                                    c
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="row justify-content-center">
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-2.3.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto" id="D">
                                    d
                                </div>
                            </div>
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-2.4.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto" id="E">
                                    e
                                </div>
                            </div>
                            <div class="card col-12 col-md-6 col-lg-3 mb-4 mb-lg-0">
                                <img src="../../img/recursos/A1-ACCESS-W-5.png" class="img-fluid utiles" alt="image-one-question-one">
                                <div class="card-body mx-auto" id="F">
                                    f
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="dropdown-header px-0">
            <span>2.b. Which activities...</span>
        </div>
        <div class="py-4">
            <div class="complete col-12 col-lg-10 col-xl-8 mx-auto position-relative">
                <span class="indent">...are good for your body?</span>
                <textarea name="A1_Access:RW2B[1]" class="px-2"></textarea>
            </div>
            <div class="complete col-12 col-lg-10 col-xl-8 mx-auto position-relative">
                <span class="indent">...are good for your brain?</span>
                <textarea name="A1_Access:RW2B[2]" class="px-2 text"></textarea>
            </div>
        </div>
    </main>
</section>