<section id="dropdown-access-writing-3" class="question dropdown dropdown-click closed col-12 col-lg-10 col-xl-8 mb-4">
    <a href="#" class="dropdown-header">
        <span>3. Match the months to the season for your country.<mark>One is done for you</mark>.</span>
        <button class="dropdown-button text-right">
            <i class="dropdown-icon fas fa-chevron-down"></i>
        </button>
    </a>
    <main class="dropdown-menu-content three-question-container px-xl-4">
        <table class="table table-normal table-striped mb-4">
            <thead>
                <tr>
                    <th scope="col" colspan="2"></th>
                    <th scope="col">Month</th>
                    <th scope="col">Season</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <th class="text-center" scope="row">1</th>
                    <td>The weather is cooler and the leaves fall off the trees</td>
                    <td><select class="input-long" disabled>
                        <option selected>April</option>
                    </select></td>
                    <td><select class="input-long" name="A1_Access:RW3[1]">
                        <option selected>Spring</option>
                        <option disabled>Summer</option>
                        <option>Autumn</option>
                        <option disabled>Winter</option>
                    </select></td>
                </tr>
                <tr>
                    <th class="text-center" scope="row">2</th>
                    <td>The weather gets really hot!</td>
                    <td><select class="input-long" name="A1_Access:RW3[2]">
                        <option selected>January</option>
                        <option>February</option>
                        <option>March</option>
                        <option disabled>April</option>
                        <option>May</option>
                        <option>June</option>
                        <option>July</option>
                        <option>August</option>
                        <option disabled>September</option>
                        <option>October</option>
                        <option>November</option>
                        <option>December</option>
                    </select></td>
                    <td><select class="input-long" disabled>
                        <option selected>Summer</option>
                    </select></td>
                </tr>
                <tr>
                    <th class="text-center" scope="row">3</th>
                    <td>It's cold and it rains a lot!</td>
                    <td><select class="input-long" name="A1_Access:RW3[3]">
                        <option selected>January</option>
                        <option>February</option>
                        <option>March</option>
                        <option disabled>April</option>
                        <option>May</option>
                        <option>June</option>
                        <option>July</option>
                        <option>August</option>
                        <option disabled>September</option>
                        <option>October</option>
                        <option>November</option>
                        <option>December</option>
                    </select></td>
                    <td><select class="input-long" disabled>
                        <option selected>Winter</option>
                    </select></td>
                </tr>
                <tr>
                    <th class="text-center" scope="row">4</th>
                    <td>Flowers start blooming.</td>
                    <td><select class="input-long" disabled>
                        <option selected>September</option>
                    </select></td>
                    <td><select class="input-long" name="A1_Access:RW3[4]">
                        <option selected>Spring</option>
                        <option disabled>Summer</option>
                        <option>Autumn</option>
                        <option disabled>Winter</option>
                    </select></td>
                </tr>
            </tbody>
        </table>
    </main>
</section>