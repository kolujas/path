<section id="dropdown-writing-4" class="question dropdown dropdown-click closed col-12 col-lg-10 col-xl-8 mb-4">
    <a href="#" class="dropdown-header">
        <span>4.a. Match the images with the words below.<mark>One is done for you</mark>.</span>
        <button class="dropdown-button text-right">
            <i class="dropdown-icon fas fa-chevron-down"></i>
        </button>
    </a>
    <main class="dropdown-menu-content four-question-container px-xl-4">
        <div class="row justify-content-md-between justify-content-lg-center pt-4">
            <ol class="col-12 mb-4 answer-words d-flex justify-content-around justify-content-lg-center">
                <li class="answers crossed inline mx-lg-2">Sunny</li>
                <li class="answers inline mx-lg-2">Cloudy</li>
                <li class="answers inline mx-lg-2">Windy</li>
                <li class="answers inline mx-lg-2">Misty</li>
            </ol>
            <div class="col-12 col-md-6 col-lg-3 mb-4 mb-lg-0 card-min-width">
                <div class="card">
                    <img src="../../img/recursos/writing-1-1.jpg" class="img-fluid" alt="image-one-question-one">
                    <div class="card-body mx-auto">
                        <select clas="input-long" disabled>
                            <option selected>Sunny</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-6 col-lg-3 mb-4 mb-lg-0 card-min-width">
                <div class="card">
                    <img src="../../img/recursos/writing-1-2.jpg" class="img-fluid" alt="image-one-question-one">
                    <div class="card-body mx-auto">
                        <select clas="input-long" name="A1_Achiever:W4A[1]">
                            <option disabled selected>Sunny</option>
                            <option>Cloudy</option>
                            <option>Windy</option>
                            <option>Misty</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-6 col-lg-3 mb-4 mb-lg-0 card-min-width">
                <div class="card">
                    <img src="../../img/recursos/writing-1-3.jpg" class="img-fluid" alt="image-one-question-one">
                    <div class="card-body mx-auto">
                        <select clas="input-long" name="A1_Achiever:W4A[2]">
                            <option disabled selected>Sunny</option>
                            <option>Cloudy</option>
                            <option>Windy</option>
                            <option>Misty</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-6 col-lg-3 mb-4 mb-lg-0 card-min-width">
                <div class="card">
                    <img src="../../img/recursos/writing-1-4.jpg" class="img-fluid" alt="image-one-question-one">
                    <div class="card-body mx-auto">
                        <select clas="input-long" name="A1_Achiever:W4A[3]">
                            <option disabled selected>Sunny</option>
                            <option>Cloudy</option>
                            <option>Windy</option>
                            <option>Misty</option>
                        </select>
                    </div>
                </div>
            </div>
        </div>
        <div class="dropdown-header px-0">
            <span>4.b. What...</span>
        </div> 
        <div class="pb-4">
            <div class="col-12 col-lg-10 col-xl-8 mx-auto position-relative">
                <span class="my-friend">...can you do on a windy day?</span>
                <textarea name="A1_Achiever:W4B[1]" class="pl-2"></textarea>
            </div>
            <div class="col-12 col-lg-10 col-xl-8 mx-auto position-relative last">
                <span class="my-friend">...do you like doing on sunny days?</span>
                <textarea name="A1_Achiever:W4B[2]" class="pl-2"></textarea>
            </div>
        </div>   
    </main>
</section>